...
Queue.prototype.pub = function(event, msg) {
  ...
  if (0 === len)
    return this.pendingPubs.push([event, buf])
...
Queue.prototype.req = function(event, msg, callback) {
  var streams = this.socket.streams
  var len = streams.length
  var stream = streams[this.n++ % len]
  if (!stream)
    return this.pendingReqs.push([event, msg, callback])
